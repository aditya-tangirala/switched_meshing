...
int main (int argc, char *argv[])
{
    ...
    pointToPointLeaf.SetDeviceAttribute	("DataRate",	StringValue	("400Mbps"));
    pointToPointLeaf.SetChannelAttribute("Delay",	StringValue	("1000ms"));


    PointToPointHelper  edgeSwitches;
    edgeSwitches.SetDeviceAttribute ("DataRate",    StringValue ("1000Mbps"));
    edgeSwitches.SetChannelAttribute("Delay",   StringValue ("400ms"));
    ...
    Address sinkLocalAddress(InetSocketAddress  (d.GetSpokeIpv4Address(4), port));
     ...
     for (uint32_t j = 0; j < nLeaf; ++j)
        {
			...
			NodeContainer n04= NodeContainer(d.GetSpokeNode(j),nodes.Get(4));
  			...
  			InternetStackHelper outerstck;
  			outerstck.Install(nodes);
            ...
            ipv4.Assign (d03);
            ipv4.Assign (d04);			
            NodeContainer cn= NodeContainer(d.GetSpokeNode(j),d.GetSpokeNode(j+1));
            NetDeviceContainer dn=edgeSwitches.Install(cn);
            ipv4.Assign (dn);
            }
            else if(j==1)
            ...
            ipv4.Assign (d03);
            ipv4.Assign (d04);
            NodeContainer cn= NodeContainer(d.GetSpokeNode(j),d.GetSpokeNode(0));
            NetDeviceContainer dn=edgeSwitches.Install(cn);
            ipv4.Assign (dn);
            }
